            ...
            {isCheckedOverlay && (
              <div className='z-50 absolute w-[896.1px] grid h-[1000px] place-items-center'>
                <div className='flex flex-col items-center justify-center text-center gap-y-8 htmlrender-detail'>
                  <h1 className='text-white text-[4.3rem] font-semibold w-full leading-[5rem]' id='title'>{isCheckedTitle ? ( <div><HtmlRenderer html={inputEventDescTitle} /></div> ) : null} </h1>
                  {isCheckedHastag ? (<p className='w-fit text-[1.8rem] bg-white text-rose-500 w-fit px-6 py-3 rounded-full text-[#E74C3C] font-semibold factshadow'> <div><HtmlRenderer html={inputEventDescHastag} /></div> </p>) : null}
                  <p className='text-white w-full font-base text-[2rem] font-semibold'>{isCheckedSubTitle ? ( <div className='flex flex-row items-center w-full justify-center gap-x-2'><HtmlRenderer html={inputEventDescSubTitle} /> <ArrowRight className='w-7 h-7' /> </div> ) : null}</p>
                </div>
              </div>
            )}
