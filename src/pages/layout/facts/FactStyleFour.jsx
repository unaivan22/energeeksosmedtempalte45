...
function FactStyleFour() {
  ...
  const [isCheckedSubTitle, setIsCheckedSubTitle] = useState(false);
  const [isCheckedHastag, setIsCheckedHastag] = useState(false);
  ...
  const [inputEventDescSubTitle, setInputEventDescSubTitle] = useState('Geser');
  const [inputEventDescHastag, setInputEventDescHastag] = useState('#Techfact');
  ...
  const handleInputChangeEventHastag = (value) => {
    setInputEventDescHastag(value);
  };
      ...
      <div className='relative gap-6 mt-12'>
        ...
        <div className={`safe-area flex items-start justify-start w-full scale-[.7] -translate-y-[8vh] -translate-x-[10vw]`}>
          <div className={`w-[1000px] h-[1000px] overflow-hidden relative shadow-2xl flex flex-col items-center justify-center bg-white renderthis` } ref={elementRef}>
            ...
            {isCheckedOverlay && (
              <div className='z-50 absolute w-[896.1px] grid h-[1000px] place-items-center'>
                <div className='flex flex-col items-center justify-center text-center gap-y-8 htmlrender-detail'>
                  <h1 className='text-white text-[54px] font-semibold w-full leading-[4rem]' id='title'>{isCheckedTitle ? ( <div><HtmlRenderer html={inputEventDescTitle} /></div> ) : null} </h1>
                  {/* {isCheckedHastag ? (<p className='w-fit text-[1.5rem] bg-white text-rose-500 w-fit px-6 py-3 rounded-full text-[#E74C3C] font-semibold factshadow'> <div><HtmlRenderer html={inputEventDescHastag} /></div> </p>) : null} */}
                  {/* <p className='text-white w-full font-base text-[1.8rem] font-semibold'>{isCheckedSubTitle ? ( <div className='flex flex-row items-center w-full justify-center gap-x-2'><HtmlRenderer html={inputEventDescSubTitle} /> <ArrowRight className='w-7 h-7' /> </div> ) : null}</p> */}
                </div>
              </div>
            )}

            {isCheckedOverlay? ( 
            <div className='absolute bottom-0 left-0 h-full w-full dot-pattern'>
              <div className='bg-gradient-to-t from-[#E74C3C] via-[#E74C3C] to-[#E74C3C] h-full opacity-80 '></div>
            </div>
            ) : null} 

          {isCheckedOverlay && (
            <div className='absolute bottom-16 w-[1000px]'>
                <p className='text-white w-full text-[1.4rem] font-light text-center'> {isCheckedCopyright ? ( <div className='flex items-center gap-2 justify-center'><Camera className='w-8 h-8' /> <HtmlRenderer html={inputEventDescCopyright} /></div> ) : null}</p>
            </div>
            )}
            ...
            <div className='flex flex-col gap-y-3'>
              ...
              <Tabs defaultValue="title" className="w-full">
                <TabsList className="grid w-full grid-cols-2 mb-6">
                  ...
                  {/* <TabsTrigger value="hastag">Hastag</TabsTrigger>
                ...
                {/* <TabsContent value="hastag">
                  <div className='flex flex-col gap-1'>
                    <div className="flex items-center">
                        <label className="inline-flex items-center cursor-pointer mb-1">
                          <input type="checkbox" value="" className="sr-only peer"  checked={isCheckedHastag} onChange={() => setIsCheckedHastag(!isCheckedHastag)} />
                          <div className="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-green-300 dark:peer-focus:ring-green-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-green-600"></div>
                          <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">Hastag</span>
                        </label>
                    </div>
                    {isCheckedHastag? ( 
                        <ReactQuill
                        theme="snow"
                        value={inputEventDescHastag}
                        onChange={handleInputChangeEventHastag}
                        modules={{ toolbar: fullToolbarOptions }}
                        className='quill-editor rounded-xl bg-white h-[250px] overflow-y-scroll'
                        />
                      ) : null} 
